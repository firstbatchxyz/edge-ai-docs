...
You should test these models on your hardware using the `dkn-compute-launcher measure` command to ensure they meet the performance requirements. The model names for use with the launcher are:
...
<Note>
API-based models (like those from Gemini, OpenAI, and Claude) do not require local measurement with the `measure` command, as their performance depends on the API provider, not your local hardware. You only need to measure locally hosted Ollama models.
</Note>
